...
public class AttachSourceActionTest {
    ...
    public void testRunWithMultipleRootSelectionAttachesSourceToAll() throws IOException, CoreException, InterruptedException {
        ...
        SingleJarProjectSetup setup1 = createJavaProjectWithLibrary("attach-source-action-multi-1-" + UUID.randomUUID(), jar1); //$NON-NLS-1$
        SingleJarProjectSetup setup2 = createJavaProjectWithLibrary("attach-source-action-multi-2-" + UUID.randomUUID(), jar2); //$NON-NLS-1$
    ...
    private File createJarCopy(File binaryJar, String copyName) throws IOException {
        ...
        FileUtils.copyFile(binaryJar, copy);
