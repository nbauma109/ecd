...
## Why a fork?
  ...
  * Code realignment for debugging uses [jd-util](https://github.com/nbauma109/jd-util)'s [JAVACC](https://javacc.github.io/javacc/) parser instead of upstream's `LineReformatter`, providing a full parse/realign phase with realignment status reporting (natively realigned, parsed and realigned, or parse error)
  * Source attach enhancements over upstream's original plugin: local source lookup from Maven (`~/.m2/repository`) and Gradle (`~/.gradle/caches`) caches before querying remote repositories, configurable public repository list via preferences (instead of hardcoded), JitPack and Sonatype Central support, and private Nexus repository with credentials
